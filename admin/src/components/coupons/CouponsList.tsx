...
export function CouponsList() {
  ...
  const load = async () => {
    setLoading(true);
    try { const result = await couponService.getAll(); setCoupons(Array.isArray(result) ? result : []); }
    catch { toast.error('Failed to load coupons'); setCoupons([]); }
  ...
  const openEdit = (c: Coupon) => {
    ...
    setEditId(c._id || c.id || ''); setModalOpen(true);
  };
    ...
    { key: 'expiresAt', header: 'Expires', render: (c) => <span className="text-xs text-muted-foreground">{formatDate(c.expiresAt)}</span> },
    {
      ...
      render: (c) => (
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="icon-sm" className="hover:border-destructive hover:text-destructive" onClick={() => setDeleteId(c._id || c.id || null)}><Trash2 className="h-3.5 w-3.5" /></Button>
        </div>
      ),
    },
  ];
  ...
  return (
    <div>
      ...
      <DataTable columns={columns} data={filtered} loading={loading} searchValue={search} onSearchChange={setSearch} searchPlaceholder="Search coupons..." rowKey={(c) => c._id || c.id || ''} />
