...
class PosController {
  async createOrder(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const { items, customerName, customerPhone, customerEmail, paymentMethod, posDiscount, posDiscountType, notes } = req.body;
      ...
      for (const item of items) {
        ...
        orderItems.push({ product_id: product.id, product_name: product.name, product_image: product.images?.[0] || null, quantity: item.quantity, price, total: price * item.quantity });
      ...
      const { data: order, error } = await supabase.from('orders').insert({
        ...
        gst_amount: gstAmount, gst_rate: enableGst ? gstRate : 0, invoice_number: invoiceNumber,
      ...
      sendCreated(res, { order: t, billing, gstAmount, gstRate: enableGst ? gstRate : 0, invoiceNumber }, 'POS order created and stock updated');
