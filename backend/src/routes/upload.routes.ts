...
import { isAdmin, canUpload } from '../middleware/role.middleware';
...
router.post('/', authenticate, canUpload, upload.single('image'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.file) {
...
router.post('/multiple', authenticate, canUpload, upload.array('images', 10), async (req: Request, res: Response, next: NextFunction) => {
  try {
